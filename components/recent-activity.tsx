  ...
  if (loading) {
    return (
      <div className="bg-card rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-border shadow-sm">
        <h2 className="text-base sm:text-lg font-semibold text-foreground mb-4 sm:mb-6">Recent Activity</h2>
    ...
    <div className="bg-card rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-border shadow-sm">
      ...
      <div className="space-y-3 sm:space-y-4">
        {activities.map((activity) => {
          ...
          return (
            <div key={activity.id} className="flex items-start gap-3 sm:gap-4 pb-3 sm:pb-4 border-b border-border/50 last:border-0 last:pb-0">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 sm:gap-3">
                  <div className="min-w-0">
                    <p className="text-xs sm:text-sm text-foreground line-clamp-2 sm:line-clamp-none">
                      <span className="font-medium">{activity.user.name}</span>
                      <span className="text-muted-foreground hidden sm:inline"> {activity.action}</span>
                      <span className="text-muted-foreground sm:hidden text-[11px]"> {activity.action}</span>
                    </p>
                    <p className="text-xs text-muted-foreground mt-1 truncate">{activity.project.name}</p>
                  </div>
                  <Icon className={`w-4 h-4 ${color} mt-1 flex-shrink-0`} />
                </div>
              </div>

              <span className="text-xs text-muted-foreground flex-shrink-0 whitespace-nowrap">{timeAgo}</span>
            </div>
          );
        })}
      </div>
